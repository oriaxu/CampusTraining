public class Main {
    ...
    static void func(int A[]) {
        ...
        /*还记得编程之美中的话吗？
        我们的程序不需要按照上述递推公式计算每个集合，只需要为每个集合设一个标志数组，标记SUM/2到1这个区间中的哪些值可以被计算
        出来。
        ...
        // 重点来了
        for (int k = 0; k < n2; k++) {
            //i取k和n中的较小值，我们的目的是找出集合S(2N, N)中与SUM最接近的那个和，所以k>n时，取到n就足够了。k<n时，
            // 我们显然无法从3个数中任意选择4个数，所以取k值
            for (i = k > n ? n : k; i >= 1; i--) {
                // 两层外循环是遍历集合S(k,i)，遍历顺序S[1][1],S[2][2],S[2][1],S[3][3]……特殊的依赖关系导致必须这样设计算
                // 法
                // 内层循环计算将A[k]加入到集合中能取到的可能的j值
                for (j = 0; j <= sum / 2; j++) {//j是i个任意整数可能的和，从0遍历到sum / 2，判断能否得到
                    // 得到j值的条件，j是和，A[k]只是其中一个，肯定需要j >= A[k]，否则，取flag[i - 1][j - A[k]]的值的时候
                    // 会发生越界情况。flag[i - 1][j - A[k]] = true代表可以找到i - 1个数，使它们的和为j - A[k]，
                    // 所以此条件满足时意味着flag[i][j] = true
